...
public class Player2Controller : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        controls.Player2.Movement.performed += ctx => movement = ctx.ReadValue<float>();
        controls.Player2.Movement.canceled += _ => movement = 0;
        controls.Player2.Jump.started += StartCoroutine_Auto => Jump();
        controls.Player2.Jump.started += o => jumping = true;
        controls.Player2.Jump.canceled += o => jumping = false;
        controls.Player2.LightAttack.started += v => LightAttack();
        controls.Player2.Crouch.started += g => crouching = true;
        controls.Player2.Crouch.canceled += g => crouching = false;
        controls.Player2.Special.started += y => special(); 
        controls.Player2.Defense.started += z => defense();
        controls.Player2.Defense.started += j => defending = true;
        controls.Player2.Defense.canceled += j => defending = false;
    }
    ...
    void Update()
    {
        ...
        if (rotateheli)
        {
            ...
            if (heliprop.transform.position.y < 1.159f)
                helihat.transform.parent.GetComponent<Rigidbody2D>().AddForce(Vector2.up * 2f);
        }
    }
    ...
    private void defense()
    {
        ...
        if (controls.Player2.Movement.ReadValue<float>() < 0 && canAttack) //Dashing left
        {
            canAttack = false;
        ...
        else if (controls.Player2.Movement.ReadValue<float>() > 0 && canAttack) //Dashing right
        {
            canAttack = false;
            ParticleSystem clone1 = Instantiate(puff, transform.position, new Quaternion(0, 0, 0, 0));
    ...
    {
        if (controls.Player2.Movement.ReadValue<float>() == 0 && isGrounded && !crouching && canAttack) //Small punch
        {
            canAttack = false;
            arm.SetActive(true);
        ...
        else if (!isGrounded && controls.Player2.Movement.ReadValue<float>() == 0 && !crouching && canAttack) //Small kick
        {
            canAttack = false;
            foot.SetActive(true);
        ...
        else if (controls.Player2.Movement.ReadValue<float>() > 0 && isGrounded && !crouching && canAttack || controls.Player2.Movement.ReadValue<float>() < 0 && isGrounded && !crouching && canAttack)//Slide kick
        {
            canAttack = false;
        ...
        else if (!isGrounded && controls.Player2.Movement.ReadValue<float>() > 0 && !crouching && canAttack) //Spin right
        {
            canAttack = false;
        ...
        else if (!isGrounded && controls.Player2.Movement.ReadValue<float>() < 0 && !crouching && canAttack) //Spin left
        {
            canAttack = false;
            rb.constraints -= RigidbodyConstraints2D.FreezeRotation;
            rb.AddTorque(-200);
    ...
    private void special()
    {
        ...
        else if (controls.Player2.Movement.ReadValue<float>() < 0 && !crouching && canAttack) //Shopping cart left
        {
            canAttack = false;
        ...
        else if (controls.Player2.Movement.ReadValue<float>() > 0 && !crouching && canAttack) //Shopping cart right
        {
            canAttack = false;
            GameObject card = Instantiate(cart, new Vector3(transform.position.x + 0.5f, transform.position.y, transform.position.z), new Quaternion(0,-0.707106829f,0,0.707106829f));
        ...
        else if (!isGrounded && canAttack && jumping) //Recovery
        {
